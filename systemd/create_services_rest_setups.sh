...
SERVICES_PORTS=(
  ["auth"]=$AUTH_PORT #REST
  ["redis_connector"]=$REDIS_CONNECTOR_PORT #REST
...
for SERVICE_NAME in "${!SERVICES_PORTS[@]}"; do
 echo "Service: $SERVICE_NAME, Port: '${SERVICES_PORTS[$SERVICE_NAME]}'"
  ...
  # can be changed if we dont want to use okx env in the future
    ENV_PATH="$venv"
    ENV_PATH_STR="\$venv"  # Use the okxenv environment for okx services
...
ExecStart=$ENV_PATH/bin/gunicorn -w 1 -b 0.0.0.0:$PORT --pid $PID_FOLDER/$SERVICE_NAME.pid --access-logfile $PID_FOLDER/$SERVICE_NAME_access.log --error-logfile $PID_FOLDER/$SERVICE_NAME.log app.$SERVICE_NAME:app
