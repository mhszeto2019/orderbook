...
class TraderNotifier:
    ...
    def __init__(self,username, ACCOUNT_SID, AUTH_TOKEN, TWILIO_PHONE, YOUR_PHONE,HTX_APIKEY,HTX_SECRETKEY,OKX_APIKEY,OKX_SECRETKEY,OKX_PASSPHRASE,DERIBIT_APIKEY,DERIBIT_SECRETKEY,BINANCE_APIKEY,BINANCE_SECRETKEY):
        ...
        self.deribit_apikey =    DERIBIT_APIKEY
        self.deribit_secretkey = DERIBIT_SECRETKEY
        self.binance_apikey =    BINANCE_APIKEY
        self.binance_secretkey = BINANCE_SECRETKEY

        self.deribit_exchange = ccxt.deribit({
                                    'apiKey':   self.deribit_apikey,
                                    'secret':   self.binance_secretkey,
                                })
        self.binance_exchange = ccxt.binancecoinm({
                                    'apiKey':   self.binance_apikey,
                                    'secret':   self.binance_secretkey,
                                })
        self.binance_exchange.options['portfolioMargin'] = True
        self.okx_exchange     = ccxt.okx({
                                    'apiKey':  self.okx_apikey,
                                    'secret':  self.okx_secretkey,
                                    'password':self.okx_passphrase
                                })
        self.htx_exchange     = ccxt.huobi({
                                    'apiKey': self.htx_apikey,
                                    'secret': self.htx_secretkey,
                                    'options': {
                                        'defaultType': 'swap',
                                    },
                                })
...
cursor.execute("""select
                ...
                MAX(CASE WHEN exchange = 'deribit' THEN apikey END) AS deribit_apikey,
                MAX(CASE WHEN exchange = 'deribit' THEN secretkey END) AS deribit_secretkey,
                MAX(CASE WHEN exchange = 'binance' THEN apikey END) AS binance_apikey,
                MAX(CASE WHEN exchange = 'binance' THEN secretkey END) AS binance_secretkey
...
for account in accounts:
    ...
    trader_notifier = TraderNotifier(account[0],ACCOUNT_SID, AUTH_TOKEN, TWILIO_PHONE, account[1],account[2],account[3],account[4],account[5],account[6],account[7],account[8],account[9],account[10])
