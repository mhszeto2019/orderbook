...
// function updateBidAndAskPrices(exchange,ccy,data) {
//     const key = `${exchange}_${ccy}`;
//     // Check if the row for this pair already exists
//     let row = document.getElementById(key);

//         // If the row exists, update only bidPrice and askPrice
//         if (row) {
//             // Update bidPrice and askPrice only
//             row.querySelector('[data-key="bidPrice"]').innerText = data.bidPrice || '-';
//             row.querySelector('[data-key="askPrice"]').innerText = data.askPrice || '-';
//             // row.querySelector('[data-key="bidSize"]').innerText = data.bidSize || '-';
//             // row.querySelector('[data-key="askSize"]').innerText = data.askSize || '-';
