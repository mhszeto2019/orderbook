...
function handlePriceUpdate(websocket_connection_name,message){
    ...
    if (!message.event){
        ...
        const time_of_snaphsot = new Date().toLocaleString();

        document.getElementById(`last-updated-${channel}`).innerHTML = time_of_snaphsot
        ...
        else if (channel =="positions")
            {
               ...
               // Assuming data is an array of objects
                ...
                data.forEach((data) => {
                    // Calculate liqPrice - markPx for each row
                    const liqPriceDifference = (parseFloat(data.liqPx) - parseFloat(data.markPx)).toFixed(6);
                    const updatedTime = Date(data.uTime).toLocaleString() 
                    // const updatedTime = new Date(data.uTime).toLocaleTimeString();
                    // const updatedTime = new Date(data.uTime * 1000).toLocaleTimeString();
                    // const updatedTime = new Date(data.uTime * 1000).toLocaleTimeString("sg-SG");
